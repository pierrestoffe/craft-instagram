...
class Media extends Component
{
    ...
    public function getHtmlFromUrls($urls)
    {
        ...
        foreach ($urls as $url) {
            ...
            // Check first for this media information in the cache
            $id = $matches[2];
            $cachedMedia = Craft::$app->getCache()->get('instagram-id-html-' . $id);
            if ($cachedMedia !== false) {
                $allMedia[] = $cachedMedia;
                continue;
            }
            ...
            $mediaInformationHtml = $mediaInformation['html'];

            // Save media information in cache
            Craft::$app->getCache()->set('instagram-id-html-' . $id, $mediaInformationHtml, 3600);

            $allMedia[] = $mediaInformationHtml;
        }
